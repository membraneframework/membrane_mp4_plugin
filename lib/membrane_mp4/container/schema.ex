defmodule Membrane.MP4.Container.Schema do
  ...
  A schema describes the valid box hierarchy of an MP4 file — which boxes exist,
  what fields they contain, and how they nest. Use `#{inspect(__MODULE__)}.Parser`
  to compile a `t:schema_def_t/0` into a `t:t/0`, or obtain the built-in default
  via `#{inspect(__MODULE__)}.Default.schema/0`.
  """
  ...
  defp do_parse(schema_def) do
    Map.new(schema_def, &parse_box/1)
  end

  @non_box_keys [:version, :fields, :black_box?]
  defp extract_box_names(schema_def) do
    Enum.flat_map(schema_def, fn {key, value} ->
      cond do
        key in @non_box_keys -> []
        is_list(value) -> [Atom.to_string(key) | extract_box_names(value)]
        true -> []
      end
    end)
  end

  defp parse_box({name, schema_def}) do
    schema =
      if schema_def[:black_box?] do
        ...
        schema
        |> Map.new()
        |> Map.merge(%{black_box?: false, children: do_parse(children)})
